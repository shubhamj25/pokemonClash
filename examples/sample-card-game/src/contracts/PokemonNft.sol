...
contract PokemonNft is ERC721, ERC721Enumerable {
    ...
    function registerPlayerCards() public {
        if (balanceOf(msg.sender) == 0) {
            for (uint256 i = 0; i < 10; i++) {
                _tokenIdCounter.increment();
                uint256 tokenId = _tokenIdCounter.current();
                mint(msg.sender, tokenId, getPokemon(randMod(100)));
                playerOwnedNfts[msg.sender].push(tokenId);
            }
        }
    }
    ...
    constructor() ERC721("Pokemon Clash", "GM") {
